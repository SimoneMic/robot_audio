...
import torch
import torchaudio
from silero_vad import VADIterator, utils_vad
...
def main(ARGS):
    ...
    for frame in frames:
        if frame is not None:
            ...
            time_stamps = utils_vad.get_speech_timestamps(audio_float32, model,
                                                            )

            if(len(time_stamps)>0):
                print(f"silero VAD has detected a possible speech {time_stamps=}")
            else:
                print(f"silero VAD has detected a noise")
            print()
            wav_data = bytearray()
